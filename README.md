...
- **🏆 实测排行榜（/stats）**：全网用户匿名共享数据汇总的大模型速度排行（中位/平均 TPS、首响、峰值、样本数）
- **🖼 一键导出长图**：标题/备注/水印/卡片渲成 2x PNG，直接发文
- **📊 紧凑模式**：折叠输出只看指标，多模型纯竞速一屏看全
- **字数核对**：Prompt 含「N 字」要求时显示达成率，揭穿字数谎报
- **🌙 暗色主题**：一键切换，记忆偏好，刷新无闪烁
- 公网部署内置 IP 限流（防代理被白嫖）与 Vercel Analytics（PV/UV）
