...
test("review draft prompt carries metrics, prompt, outputs, and anti-hallucination rules", () => {
  ...
  assert.match(prompt, /不要虚构任何未提供的指标/);
  assert.match(prompt, /首 Token：0\.62s/);
  assert.match(prompt, /输出 TPS：88/);
  assert.match(prompt, /输出 Tokens：160（官方）/);
